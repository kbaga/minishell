...
void execute_command(t_exec *cmd) 
{
	...
	if (is_builtin(cmd->execs[0]))
		execute_builtin(cmd->execs[0], cmd->execs);
	else	
...
void execute_command(t_exec *node, t_env_list *env_list)
{
	...
	resolved_path = my_get_path(node->execs[0], env_list);
	if (!resolved_path)
	...
	{
		free(resolved_path);
		error_command("execve");
	}
	free(resolved_path); // Free resolved path (not reached if execve succeeds)
}

void execute_external(t_exec *head, t_env_list *env_list)
{
	t_exec *current;
	pid_t pid;
	...
	while (current)
	{
		...
		if (pid < 0) 
		{
			...
			execute_command(current); // Execute command in child
		else
		{
...
int main(int argc, char **argv, char **env) {
    ...
    cmd3.fd_in = 0;
    cmd3.fd_out = 1;
    cmd3.execs = (char *[]){"wc", "-l", NULL};
    cmd3.next = NULL;
    cmd3.prev = &cmd2;

    // Execute the command list
    execute_exec_list(&cmd1);
